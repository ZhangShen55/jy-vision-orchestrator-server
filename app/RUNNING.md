...
| 入口 | 命令 | 作用 |
| --- | --- | --- |
| API 控制面 | `python -m app.main --config <config> serve` | 接收 TIAS 注册心跳，查询状态，修改 Worker 集群期望状态 |
| 受控 Kafka Worker | `python -m app.main --config <config> worker` | 按 Redis `desired_state` 消费 `classroom_cv_task`，抽帧后调度 TIAS |
| 兼容 Kafka Worker | `python -m app.main --config <config> consume` | 旧入口，启动后直接消费 Kafka，不受 Redis 控制 |
...
```bash
export CONFIG_PATH="$PWD/app/config.toml"
python -m app.main --config "$CONFIG_PATH" serve
```
...
python -m app.main --config "$CONFIG_PATH" worker
```
...
```bash
python -m app.main --config "$CONFIG_PATH" consume
```
...
```bash
docker build -f app/docker/Dockerfile -t vision-orchestrator:6.0 .
```

构建 Cython 保护镜像：

```bash
docker build -f app/docker/Dockerfile \
...
docker run -d \
  ...
  -v "$PWD/app/config.toml:/app/config.toml:ro" \
  -v "$PWD/mnt:/mnt" \
...
docker run -d --name vision-orchestrator-api-1 -p 9001:9000 \
  -v "$PWD/app/config.toml:/app/config.toml:ro" \
  -v "$PWD/mnt:/mnt" \
  vision-orchestrator:6.0 python -m app.main --config /app/config.toml serve
...
```bash
python -m app.main --config "$CONFIG_PATH" run-json tests/fixtures/vision_orchestrator_lesson_message.json
```
...
```bash
python - <<'PY'
from app.infrastructure.tias.registry import RedisTiasRegistry
